    ...
    @if (isLoggedIn) {
      ...
      @if (!open) {
        <button (click)="open = true"
                class="fixed bottom-6 right-6 z-50 w-12 h-12 rounded-full bg-sky-500 hover:bg-sky-600 shadow-lg text-white flex items-center justify-center transition-colors cursor-pointer">
      ...
      @if (open && !showSaveModal) {
        <div class="fixed bottom-6 right-6 z-50 w-80 h-[28rem] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col">
          <!-- Header -->
          <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <div>
              <span class="text-sm font-semibold text-gray-800">Ask anything</span>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm"
          ...
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden"
            ...
            <div class="px-5 pt-5 pb-3">
              <h3 class="text-lg font-semibold text-gray-900">Save as flashcards?</h3>
              <p class="text-sm text-gray-400 mt-1">Select which terms to save</p>
            </div>

            <!-- Term checklist -->
            <div class="max-h-48 overflow-y-auto px-5 space-y-2">
              @for (item of collected; track $index) {
                <label class="flex items-start gap-2 cursor-pointer py-1">
                  <input type="checkbox" [(ngModel)]="item.selected"
                         class="mt-1 rounded border-gray-300 text-sky-500 focus:ring-sky-400" />
                  <div class="min-w-0">
            ...
            <!-- Destination -->
            <div class="px-5 py-3 border-t border-gray-100 mt-2">
              <label class="block text-xs font-medium text-gray-500 mb-2">Save to</label>
              <div class="flex flex-col gap-1.5">
                @if (conceptId) {
                  <button (click)="saveTarget = 'note'"
                          [class]="'text-left px-3 py-2 rounded-lg text-sm transition-colors ' + (saveTarget === 'note' ? 'bg-sky-50 text-sky-700 font-medium' : 'text-gray-600 hover:bg-gray-50')">
                    This note
                  </button>
                }
                <button (click)="saveTarget = 'general'"
                        [class]="'text-left px-3 py-2 rounded-lg text-sm transition-colors ' + (saveTarget === 'general' ? 'bg-sky-50 text-sky-700 font-medium' : 'text-gray-600 hover:bg-gray-50')">
                  General (Q&A)
                </button>
                @if (topics.length > 0) {
                  <select [(ngModel)]="selectedTopicId" (ngModelChange)="saveTarget = 'subject'"
                          class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-sky-400 cursor-pointer">
